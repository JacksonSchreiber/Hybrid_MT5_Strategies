...
(function () {
  ...
  window.HybridChart = {
    mount: function (el, d) {
      var tf = 'h4', chart, series = [], box = el.querySelector('.hc-box'), bar = el.querySelector('.hc-bar');
      function build() {
        if (chart) { chart.remove(); }
        var bars = (tf === 'h4' ? d.bars_h4 : d.bars_d1) || [];
        ...
        var cs = chart.addCandlestickSeries({ upColor: '#3fb950', downColor: '#f85149', borderUpColor: '#3fb950', borderDownColor: '#f85149', wickUpColor: '#3fb950', wickDownColor: '#f85149',
        ...
        [20, 50, 200].forEach(function (n) {
          ...
          ls.setData(bars.map(function (b, i) { return { time: b[0], value: e[i] }; }).slice(n));
        });
        var lv = d.levels || {};
      ...
      bar.querySelectorAll('button').forEach(function (b) { b.onclick = function () { tf = b.dataset.tf; bar.querySelectorAll('button').forEach(function (x) { x.classList.toggle('on', x === b); }); build(); }; });
      build();
    }
  };
})();
