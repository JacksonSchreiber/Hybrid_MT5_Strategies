...
mkdir -p "$SETDIR"
{
  ...
  echo "InpShowEvents=true"       # event lines ON. Labels carry the event name + bias only —
                                  # the currency prefix is stripped (would leak the pair); the
                                  # advisor already has calendar impact/timing via Tier-0.
