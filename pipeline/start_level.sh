...
#   2b. Boots the inbox bridge: each fired setup is auto-delivered to the blind
#      advisor's inbox (H4+overlays cropped, D1 rendered, blind numbers). Stops
#      itself when you close the tester.
...
# --- 2b. auto-boot the inbox bridge -----------------------------------------
# While the level runs, deliver each fired setup to the blind advisor's inbox:
# H4 screenshot (overlays kept, title/time bands cropped), a blind D1 render,
# and blind numbers. The watcher self-terminates when the tester closes.
BRIDGE_LOG="$MT5_DATA/inbox_bridge.log"
if command -v python3 >/dev/null 2>&1; then
  log "inbox bridge: blind setup shots -> advisor inbox (log: $BRIDGE_LOG)"
  (
    : > "$BRIDGE_LOG"
    python3 "$HERE/inbox_bridge.py" --watch >>"$BRIDGE_LOG" 2>&1 &
    bpid=$!
  ...
  log "WARNING: python3 not found - inbox bridge NOT started (setups won't auto-deliver)."
fi
