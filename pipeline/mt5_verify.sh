...
MODE=ALL; STRAT="SMC,Fib,EMA"; SYMBOL="EURUSD.dk"; EXPERT="HybridForwardTest"
...
while [[ $# -gt 0 ]]; do case "$1" in
  ...
  --expert) EXPERT="$2"; shift 2;;   # e.g. HybridForwardTest-live (Phase-3 -live build parity runs)
...
USE_SMC=$(has SMC); USE_FIB=$(has Fib); USE_EMA=$(has EMA); USE_SHOCK=$(has Shock); USE_ERI=$(has EmaRevInv); USE_TC=$(has TrendCont); USE_EMAQ=$(has EMArevQ)
...
mkdir -p "$SETDIR"
{
  ...
  echo "InpUseEMArevQ=$USE_EMAQ"
  echo "InpUseShock=$USE_SHOCK"
  echo "InpUseEmaRevInv=$USE_ERI"
  echo "InpUseTrendCont=$USE_TC"
...
JPFX=""; [[ "$AA" != 0 ]] && JPFX="AA_"
# The MAIN journal only: never the .actions.csv / .inv.* siblings or a .part.csv. A same-range
# re-run REWRITES an existing journal name (so it is not "new" to comm), hence the primary rule
# is "newest main journal modified since launch" and the comm diff is only a tie-breaker.
main_only(){ grep -viE '\.actions\.csv$|\.inv\.|\.part\.csv$|\.delays\.csv$' || true; }
NEWJ=$(find "$COMMON_JOURNAL" -maxdepth 1 -type f -name "${JPFX}${SYMBOL}_*.csv" -newermt "@$START" -printf '%T@ %f\n' 2>/dev/null \
       | sort -n | cut -d' ' -f2- | main_only | tail -1)
[[ -z "$NEWJ" ]] && NEWJ=$(comm -13 <(printf '%s\n' "$BEFORE") <(printf '%s\n' "$AFTER") | grep -i "^${JPFX}${SYMBOL}_" | main_only | tail -1)
[[ -z "$NEWJ" ]] && NEWJ=$(ls -t "$COMMON_JOURNAL"/${JPFX}${SYMBOL}_*.csv 2>/dev/null | xargs -r -n1 basename | main_only | head -1)
...
for s in SweepMSS DeepFib EMArev EMArevQ TrendCont ShockCont EmaRevInv; do
